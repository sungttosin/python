...
print (c) print (d) 
#가리키는대상을 찾아 들어가보면 아무것도 없음 e = None; a = 100; b = 200; print (a, b) #변수 e를 이용하여 a와 b가 가리키는 값을 서로 바꾸는 코드를 작성해 봅시다. ''' #코드 작성 a = 100; b = 200; e = b; b = a; a = e; print(a, b) #"객체" 이해하기 ''' 일상생활속에서 존재하는 물건을 '사물' 소프트웨어 속에서 존재하는 것을 '객체' 라고 부릅니다. ''' '''인간이라는 사물은 age, name, weight와 같은 어떤 값을 갖고, eat(), run(), sleep() 와 같은 행동을 한다. 즉, 사물이란는 것은 '값들'과 '행동들'로 규정하여 바라보겠다는 뜻이다. 이때 행동과 관련된것 에는 ()를넣어준다. ex) eat(), run(), sleep() ''' #자동차라는 것을 어떤 값과, 행동으로 정의해 봅시다. #값:name, 탑승인원... #행동: 가속하기(), 감속하기(), 문열기()...
